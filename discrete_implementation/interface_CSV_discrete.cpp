...
void read_csv(const string& csvSource, vector<vector<double>>& lines, bool header = true, bool index = false)
{
    string field;
    lines.clear();
    vector<double> line;
...
template <typename T> void overwrite_csv(const char* filename, vector<T> x, vector<T> y, int length){
	remove(filename);
    ofstream myfile1;
